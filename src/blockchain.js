...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            resolve(`${address}:${this._currentTime()}:starRegistry`)
        });
    }
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            let currentTime = parseInt(this._currentTime());
            if (currentTime - msgTime < 300000) {
                let verifyStatus = bitcoinMessage.verify(message, address, signature);
                if (verifyStatus) {
                    let block = new BlockClass.Block({ message, address, signature, star });
                    await this._addBlock(block);
                    resolve(block);
                } else {
                    reject(Error("Failed to verify"));
                }
            } else {
                reject(Error("Taking longer than 5 minutes, aboard!"))
            }
        });
    }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            const result = this.chain.filter(p => p.hash === hash)[0];
            if (result) {
                resolve(result);
            } else {
    ...
    getStarsByWalletAddress(address) {
        ...
        let stars = [];
        return new Promise((resolve, reject) => {
            self.chain.forEach(async (elm) => {
                let data = await elm.getBData();
                if (data.address === address) stars.push(data.star);
            });
            resolve(stars);
        });
    }
